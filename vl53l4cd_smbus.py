...
# Default configuration written to register 0x002D
_DEFAULT_CONFIG = [
    0x12, 0x00, 0x00, 0x40, 0x00, 0x00, 0x10, 0x06,
    0x10, 0x06, 0x01, 0x06, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00,
...
class VL53L4CD:
    ...
    def _write_block(self, reg, data):
        # Write in small chunks (max 30 bytes per transaction)
        offset = 0
        while offset < len(data):
            chunk = data[offset:offset + 30]
            r = reg + offset
    ...
    def _init_sensor(self):
        """Full initialization sequence for the VL53L4CD."""
        print("  [1/6] Checking sensor ID...")
        model_id = self._read16(0x010F)
        print(f"  Model ID: 0x{model_id:04X} (expected 0xEBAA)")

        print("  [2/6] Waiting for boot...")
        timeout = time.time() + 5.0
        while True:
            status = self._read8(0x00E5)
            if status != 0x00:
                print(f"  Boot status: 0x{status:02X} - OK")
                break
            if time.time() > timeout:
                print("  WARNING: Boot wait timed out, continuing anyway...")
                break
            time.sleep(0.05)
        ...
        print("  [4/6] Setting timing budget...")
        # Set inter-measurement to 0 (continuous)
        self._write32(0x006C, 0)
        time.sleep(0.05)

        # Try to set timing budget
        try:
            osc_freq = self._read16(0x0006)
            print(f"  Oscillator freq: {osc_freq}")
            if osc_freq != 0:
                macro = (2304 * (0x40000000 // osc_freq)) >> 6
                budget_ms = 50
                val_a = ((budget_ms * 16 * macro) + 500) // 1000 - 1
                val_b = ((budget_ms * 16 * macro * 12 // 10) + 500) // 1000 - 1
                self._write32(0x005E, val_a)
                self._write32(0x0060, val_b)
            else:
                print("  WARNING: osc_freq is 0, skipping timing budget")
        except Exception as e:
            print(f"  WARNING: Could not set timing budget: {e}")

        print("  [5/6] Running calibration ranging...")
        ...
        # Start ranging
        self._write8(0x0087, 0x40)
        time.sleep(0.5)  # give it time to take first measurement

        # Wait for data ready with longer timeout
        timeout = time.time() + 10.0
        ready = False
        ...
        if ready:
            ...
            self._write8(0x0086, 0x01)  # clear interrupt
        else:
            print("  WARNING: Calibration ranging did not report ready")
    ...
    def start_ranging(self):
        self._write8(0x0086, 0x01)  # clear interrupt
        self._write8(0x0087, 0x40)  # start
        time.sleep(0.05)
